...
namespace Seldon
{
  ...
  */
  ...
  Reallocate(int i, int j)
  {
    ...
    val_real.Reallocate(n); ind_real.Reallocate(n);
    val_imag.Reallocate(n); ind_imag.Reallocate(n);
    nz_real_ = 0; nz_imag_ = 0;
  }
  ...
  */
  ...
  Resize(int i, int j)
  {
	...
	new_ind_real.Reallocate(new_n); new_val_real.Reallocate(new_n);
	new_ind_imag.Reallocate(new_n); new_val_imag.Reallocate(new_n);
	
	nz_real_ = 0; nz_imag_ = 0;
	for (int k=0 ; k<min(n,new_n) ; k++)
	  {
	    new_ind_real(k).SetData(ind_real(k).GetM(),ind_real(k).GetData());
	    ind_real(k).Nullify();
	    new_val_real(k).SetData(val_real(k).GetM(),val_real(k).GetData());
	    val_real(k).Nullify();
	    nz_real_ += new_ind_real(k).GetM();
	    
	    new_ind_imag(k).SetData(ind_imag(k).GetM(),ind_imag(k).GetData());
	    ind_imag(k).Nullify();
	    new_val_imag(k).SetData(val_imag(k).GetM(),val_imag(k).GetData());
	    val_imag(k).Nullify();
  ...
  ReallocateRealRow(int i, int j)
  {
	...
	val_real(i).Reallocate(j); ind_real(i).Reallocate(j);
      }
  }
  ...
  ReallocateImagRow(int i, int j)
  {
	...
	val_imag(i).Reallocate(j); ind_imag(i).Reallocate(j);
      }
  }
  ...
  ClearRealRow(int i)
  {
    ...
    if (l > 0)
      {
	nz_real_ -= l;
	val_real(i).Clear(); ind_real(i).Clear();
      }
  }
  ...
  ClearImagRow(int i)
  {
    ...
    if (l > 0)
      {
	nz_imag_ -= l;
	val_imag(i).Clear(); ind_imag(i).Clear();
      }
  }
  ...
  SwapRealRow(int i,int i_)
  {
    if (i != i_)
      {
	int j_ = ind_real(i_).GetM(); int j = ind_real(i).GetM();
	int* tmp = ind_real(i).GetData(); ind_real(i).Nullify();
	pointer tmp_val = val_real(i).GetData(); val_real(i).Nullify();
	ind_real(i).SetData(j_,ind_real(i_).GetData());
	val_real(i).SetData(j_,val_real(i_).GetData());
	ind_real(i_).Nullify(); val_real(i_).Nullify();
  ...
  SwapImagRow(int i,int i_)
  {
    if (i!=i_)
      {
	int j_ = ind_imag(i_).GetM(); int j = ind_imag(i).GetM();
	int* tmp = ind_imag(i).GetData(); ind_imag(i).Nullify();
	pointer tmp_val = val_imag(i).GetData(); val_imag(i).Nullify();
	ind_imag(i).SetData(j_, ind_imag(i_).GetData());
	val_imag(i).SetData(j_, val_imag(i_).GetData());
	ind_imag(i_).Nullify(); val_imag(i_).Nullify();
  ...
  */
...
#ifdef SELDON_CHECK_BOUNDARIES
    if ((i < 0)||(i >= this->m_))
      throw WrongRow("Matrix_ArrayComplexSparse::index", "Index should be in [0, "
		     + to_str(this->m_-1) + "], but is equal to "
		     + to_str(i) + ".");
  ...
  AddRealInteraction(int i, int j, const T& a)
  {
    ...
    ind_real(i).Clear(); val_real(i).Clear(); n++;
		     ...
		     Vector<T,Storage1,Allocator1> val_interac)
  {
    ...
    // get already existing interactions
    int nb_new = 0; Vector<bool> new_interac(nb_interac); new_interac.Fill(true);
    int k = 0;
    ...
    if (nb_new > 0)
      {
	// some interactions doesn't exist, we add them on the correct position
	...
	for (int j = 0; j < nb_interac; j++)
	  if (new_interac(j))
	      ...
	      while ((k < n)&&(ind_real(i)(k) < col_interac(j)))
		{
	      ...
	      // the new entry
	      new_ind(nb) = col_interac(j); new_val(nb) = val_interac(j); nb++;
	    }
	...
	n += nb_new;
	ind_real(i).Clear(); val_real(i).Clear();
	ind_real(i).SetData(n, new_ind.GetData());
	val_real(i).SetData(n, new_val.GetData());
	new_ind.Nullify(); new_val.Nullify();
	nz_real_ += nb_new;
  ...
  AddImagInteraction(int i, int j, const T& a)
  {
    ...
    ind_imag(i).Clear(); val_imag(i).Clear(); n++;
		     ...
		     Vector<T,Storage1,Allocator1> val_interac)
  {
	...
	n += nb_new;
	ind_imag(i).Clear(); val_imag(i).Clear();
	ind_imag(i).SetData(n, new_ind.GetData());
	val_imag(i).SetData(n, new_val.GetData());
	new_ind.Nullify(); new_val.Nullify();
	nz_imag_ += nb_new;
  ...
  */
  ...
  SetIdentity()
  {
    this->n_ = this->m_; this->nz_real_ = this->m_;
    this->nz_imag_ = 0;
    for (int i = 0; i < this->m_; i++)
      {
	ind_real(i).Reallocate(1); ind_real(0) = i;
	val_real(i).Reallocate(1); val_real(0) = T(1);
      }
  }
